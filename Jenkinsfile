pipeline {
    agent {
        docker {
            ...
            args '-u root --rm -v /var/run/docker.sock:/var/run/docker.sock -v "$PWD":/app --env DOCKER_HOST=tcp://host.docker.internal:4243 greatnate27/recipe-app-pipeline-env:v1'
        }
    }
